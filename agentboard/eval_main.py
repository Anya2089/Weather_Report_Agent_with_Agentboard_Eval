...
def main():
    ...
    if not run_config.get("wandb", False):
        ...
        engine_name = llm_config.get("engine", llm_config.get("model_id", "unknown"))
        wandb.init(
            project=run_config["project_name"],
            name="{model_name}_{engine_name}".format(model_name=llm_config["name"], engine_name=engine_name),
