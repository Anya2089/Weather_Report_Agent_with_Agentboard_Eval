...
class WeatherEnv:
    ...
    def step(self, action, action_path=None):
        ...
        # 添加调试日志
        logger.debug(f"Raw action received: '{action}'")
        
        try:
            # 标准化动作格式 - 移除可能的"Action: "前缀
            cleaned_action = action.strip()
            if cleaned_action.startswith("Action: "):
                cleaned_action = cleaned_action[8:].strip()
            
            logger.debug(f"Cleaned action: '{cleaned_action}'")
            
            # 确保动作格式正确
            if " with Action Input: " not in cleaned_action:
                observation = "ERROR | Action format error. Expected format: '[action_name] with Action Input: [action_input]'"
                logger.error(f"Action format error: {cleaned_action}")
                done = False
                return observation, self.reward, self.done, None
            
            action_type, params = parse_action(cleaned_action)
            logger.debug(f"Parsed action_type: '{action_type}', params: {params}")
        ...
        try:
            if action_type == "get_user_current_date":
                result = self.weather_toolkits.get_user_current_date(action_path=action_path)
                # 如果result是元组，说明没有提供action_path参数，需要解包
                if isinstance(result, tuple) and len(result) == 2:
                    success, observation = result
                else:
                    observation = result
            elif action_type == "get_user_current_location":
                result = self.weather_toolkits.get_user_current_location(action_path=action_path)
                if isinstance(result, tuple) and len(result) == 2:
                    success, observation = result
                else:
                    observation = result
            elif action_type == "get_historical_temp":
                result = self.weather_toolkits.get_historical_temp(
                ...
                result = self.weather_toolkits.get_historical_rain(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    start_date=params["start_date"],
                    end_date=params["end_date"],
                ...
                result = self.weather_toolkits.get_historical_snow(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    start_date=params["start_date"],
                    end_date=params["end_date"],
                ...
                result = self.weather_toolkits.get_snow_forecast(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    start_date=params["start_date"],
                    end_date=params["end_date"],
                ...
                result = self.weather_toolkits.get_current_snow(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                ...
                result = self.weather_toolkits.get_current_temp(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    current_date=params["current_date"],
                    action_path=action_path
                ...
                result = self.weather_toolkits.get_latitude_longitude(
                    name=params["name"],
                    action_path=action_path
                )
                if isinstance(result, tuple) and len(result) == 2:
            ...
            #     result = self.weather_toolkits.get_air_quality(
            #         latitude=params["latitude"],
            #         longitude=params["longitude"],
            #         action_path=action_path
            #     )
            ...
            #         observation = result
                ...
                result = self.weather_toolkits.get_elevation(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    action_path=action_path
                )
                ...
                result = self.weather_toolkits.get_temp_forecast(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    start_date=params["start_date"],
                    end_date=params["end_date"],
                ...
                result = self.weather_toolkits.get_rain_forecast(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    start_date=params["start_date"],
                    end_date=params["end_date"],
                ...
                result = self.weather_toolkits.get_current_rain(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    current_date=params["current_date"],
                    action_path=action_path
                ...
                result = self.weather_toolkits.get_distance(
                ...
                result = self.weather_toolkits.get_historical_air_quality_index(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    start_date=params["start_date"],
                    end_date=params["end_date"],
                ...
                result = self.weather_toolkits.get_current_air_quality_index(
                    latitude=params["latitude"],
                    longitude=params["longitude"],
                    current_date=params["current_date"],
                    action_path=action_path
                ...
                result = self.weather_toolkits.get_air_quality_level(
                ...
                result = self.weather_toolkits.convert_zipcode_to_address(
                ...
                if isinstance(result, tuple) and len(result) == 2:
                    ...
                    observation = result
            elif action_type == "generate_weather_report":
                result = self.weather_toolkits.generate_weather_report(
                    location_name=params["location_name"],
                    weather_data=params["weather_data"],
                    report_type=params.get("report_type", "comprehensive"),
                    output_formats=params.get("output_formats", ["json", "markdown", "html", "text"]),
                    save_to_file=params.get("save_to_file", True),
                    base_directory=params.get("base_directory", "weather_reports"),
                    action_path=action_path
                )
                if isinstance(result, tuple) and len(result) == 2:
                    success, observation = result
                ...
                result = self.weather_toolkits.finish(answer=params["answer"], action_path=action_path)
                if isinstance(result, tuple) and len(result) == 2:
                    success, observation = result
                else:
                    observation = result
